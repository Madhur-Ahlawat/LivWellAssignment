package com.example.myandroidproject.security// Dependencies: Kotlin stdlib + Android SDK
...
object MockLocationDetector {
    ...
    fun isLocationMockedBasic(context: Context, location: Location): Boolean {
        try {
            ...
            val mockAllowed = Settings.Secure.getString(
    ...
    fun startAccelerometerMonitoring(context: Context) {
        ...
        val accel = sm.getDefaultSensor(Sensor.TYPE_LINEAR_ACCELERATION) ?: sm.getDefaultSensor(
    ...
    fun isMovementSuspiciousBySensors(
        prev: Location?,
        current: Location,
        elapsedMillis: Long
    ): Boolean {
        ...
        val maxPlausibleCarSpeed =
            60.0           // ~60 m/s (~216 km/h) -> unrealistic, tune lower (e.g., 40 m/s)
    ...
    fun startMockLocationChecks(context: Context, callback: SecurityCallback) {
        val fusedClient = LocationServices.getFusedLocationProviderClient(context)

        fusedClient.lastLocation.addOnSuccessListener { location ->
            if (location == null) {
                println("⚠️ No last location available")
                return@addOnSuccessListener
            }

            val elapsedTime = if (lastLocationTime > 0)
                System.currentTimeMillis() - lastLocationTime
            else 0L

            // Start accelerometer monitoring
            startAccelerometerMonitoring(context)

            // Known mocking app detection
            val mockingAppInstalled = isKnownMockingAppInstalled(context)
            if (mockingAppInstalled) println("⚠️ Mocking app detected!")

            // Mock location permission allowed?
            val mockPermissionAllowed = isMockAppAllowed(context)
            if (mockPermissionAllowed) println("⚠️ Mock location permission allowed!")

            // Main spoof detection
            val spoofed = isLocationLikelySpoofed(
                context,
                lastKnownLocation,
                location,
                elapsedTime
            )
            if (spoofed) println("⚠️ Possible mock location detected!") else println("✅ Location OK")

            // Sensor-based suspicious movement check
            val suspiciousMovement = isMovementSuspiciousBySensors(
                lastKnownLocation,
                location,
                elapsedTime
            )
            if (suspiciousMovement) println("⚠️ Movement suspicious by sensors!")

            // Save new location & time for next check
            lastKnownLocation = location
            lastLocationTime = System.currentTimeMillis()

            // Final decision
            val isSpoofed = spoofed || mockingAppInstalled || mockPermissionAllowed || suspiciousMovement
